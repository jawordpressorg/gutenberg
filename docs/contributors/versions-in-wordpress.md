...
# Gutenberg versions in WordPress
 -->
# WordPress に組み込まれた Gutenberg のバージョン
...
Also note that during WordPress point versions (e.g. 6.8.1, 6.8.2, etc.) additional bug fixes from later Gutenberg releases than those noted are added into the WordPress release where it is needed. So for example, WordPress 6.8 ships with Gutenberg 20.4, but 6.8.1, 6.8.2, and 6.8.3 also include very specific bug fixes from Gutenberg versions after 20.4. But the base version remains 20.4. This started to apply since Gutenberg 4.8 and WordPress 5.1 onwards.
 -->
また、WordPress のポイントバージョン (例: 6.8.1、6.8.2 など) では、必要に応じて、ここに記載されたバージョンよりも新しい Gutenberg リリースから追加のバグ修正が WordPress リリースに組み込まれることにも注意してください。例えば、WordPress 6.8 には Gutenberg 20.4 が同梱されていますが、6.8.1、6.8.2、6.8.3 には 20.4 以降の Gutenberg バージョンからの非常にピンポイントなバグ修正も含まれています。ただし、ベースバージョンは 20.4 のままです。この方式は Gutenberg 4.8 と WordPress 5.1 以降から適用されています。
