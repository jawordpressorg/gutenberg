...
Interactivity API is only available for WordPress 6.5 and above.
 ...
 -->
**注意**: Interactivity API は WordPress 6.5以降でのみ利用できます。
...
<div class="callout callout-info">
 ...
 -->
**注意**: このディレクティブは、パフォーマンス向上のため、デフォルトでは非同期で実行されます。<code>event</code> オブジェクトへの同期アクセスが必要であれば、<code>withSyncEvent()</code> を使用してアクションをラップしてください。また、同期的なイベントアクセスも必要な非同期アクションの場合は、<code>withSyncEvent()</code> を、同期 API を呼び出した後にメインスレッドへ yield する (処理を戻す) ジェネレーター関数と組み合わせることができます。
...
If you need a similar reactive callback that is not tied to a specific DOM element but runs at the store level, you can use the [`watch()` function](#watch).
 -->
同様のリアクティブなコールバックで、特定の DOM 要素に関連付けられていないが、ストアレベルで実行したい場合、`watch()` 関数を使用できます。
...
_In the `view.js` file of each block_ the developer can define both the state and the elements of the store referencing functions like actions, side effects or derived state.
 -->
_各ブロックの `view.js` ファイルの中_ で、開発者はステートと、ストアの要素の両方を定義できます。ストアの要素はアクション、副作用、派生ステートなどの関数を参照します。
...
Retrieves the context inherited by the element evaluating a function from the store. The returned value depends on the element and the namespace where the function calling `getContext()` exists. It can also take an optional namespace argument to retrieve the context of a specific interactive region.
 -->
関数を評価する要素によって継承されたコンテキストを、ストアから取得します。戻り値は要素と、`getContext()` を呼び出した関数が存在する名前空間に依存します。オプションで名前空間を引数に取り、特定の interactive 領域のコンテキストを取得できます。
...
It returns an object with two keys:
 -->
アクションがバインドされた要素、または呼び出された要素の表現を取得します。この表現は読み取り専用で、DOM 要素への参照、その prop、ローカルのリアクティブなステートを含みます。
2つのキーを持つオブジェクトを返します。
...
`attributes` is an object that contains the attributes of the element. In the button example:
 -->
`attributes` は要素の属性を含むオブジェクトです。ボタンの例では、
...
This function is analogous to `getContext()`, but with 2 key differences:
 -->
この関数は `getContext()` と似ていますが、2つの重要な違いがあります。
...
2. The object returned by `getServerContext()` is read-only.
 -->
1. [`@wordpress/interactivity-router`](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-interactivity-router/) の [`actions.navigate()`](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-interactivity-router/#actions) が呼ばれると、常に`getServerContext()` が返すオブジェクトは更新されます。これは、`actions.navigate()`によってロードされたページの **新しい** コンテキストに基づいてブロックのコンテキストを更新したい場合に便利です。この新しいコンテキストは `actions.navigate()` によって読み込まれたページの HTML 内に埋め込まれます。
2. `getServerContext()` が返すオブジェクトは読み取り専用です。

<!-- 
The server context cannot be directly used in directives, but you can use callbacks to subscribe to its changes.
 -->
サーバーコンテキストは直接ディレクティブ内で使えませんが、 コールバックを使用して、その変更を受け取れます。
...
The object returned is read-only, and includes the state defined in PHP with `wp_interactivity_state()`. When using [`actions.navigate()`](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-interactivity-router/#actions) from [`@wordpress/interactivity-router`](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-interactivity-router/), the object returned by `getServerState()` is updated to reflect the changes in its properties, without affecting the state returned by `store()`. Directives can subscribe to those changes to update the state if needed.
 -->
返されるオブジェクトは読み取り専用で、PHP 内の `wp_interactivity_state()` で定義されたステートを含みます。[`@wordpress/interactivity-router`](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-interactivity-router/) の [`actions.navigate()`](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-interactivity-router/#actions) を使用する場合、`getServerState()` が返すオブジェクトは、`store()` が返すステートに影響を与えることなく、プロパティの変更を反映して更新されます。ディレクティブはこれらの変更をサブスクライブして、必要に応じて状態を更新できます。
...
#### How server context and state merging works during navigation
 -->
#### ナビゲーション時のサーバーコンテキストとステートのマージの仕組み
...
During navigation, the data returned by both `getServerContext()` and `getServerState()` is fully replaced with the values from the new page. In contrast, the related client data (context or state) is "soft merged"—existing client-side properties are preserved, and only new properties from the server are added. This ensures that new blocks or components introduced by navigation can initialize with server-provided values, while client-side changes made by users remain intact. If you need to update existing client properties with data from the server (i.e., overwrite values), call `getServerContext()` or `getServerState()` within your callbacks and manually overwrite the relevant properties.
 -->
ナビゲーション中、`getServerContext()` と `getServerState()` が返すデータは、新しいページの値で完全に置き換えられます。
一方、それに対応するクライアント側のデータ (コンテキスト、またはステート) は「ソフトマージ」されます。つまり、既存のクライアント側プロパティは保持され、サーバーから提供された新しいプロパティだけが追加されます。これにより、ナビゲーションによって導入された新しいブロックやコンポーネントはサーバーから提供された値で初期化される一方で、ユーザーがクライアント側で行った変更は保持されます。サーバーから取得したデータで既存のクライアントプロパティを更新したい場合 (つまり、既存の値を上書きしたい場合) は、コールバック内で `getServerContext()` または `getServerState()` を呼び出し、該当するプロパティを手動で上書きしてください。
...
If you subscribe to any value returned by `getServerContext()` or `getServerState()` within a callback, that callback will be invoked on every navigation event—regardless of whether that value have changed. This makes it possible to reliably reset or update client-side data as needed whenever navigation occurs.
 -->
コールバック内で `getServerContext()` または `getServerState()` が返す値をサブスクライブしている場合、そのコールバックは、その値が実際に変更されたかどうかにかかわらず、ナビゲーションが発生するたびに呼び出されます。これにより、ナビゲーションが発生するたびに、必要に応じてクライアント側のデータを確実にリセットまたは更新できます。
...
You can also use `withSyncEvent()` with generator functions (async actions). This is useful when you need both synchronous event access and asynchronous operations:
 -->
また `withSyncEvent()` はジェネレーター関数 (非同期関数) と使用できます。これは、同期イベントアクセスと、非同期操作の両方が必要な場合に便利です。
...
store( 'myPlugin', {
	actions: {
		// 非同期操作のため、withSyncEvent をジェネレーター関数と組み合わせている。
...
Subscribes to changes in any signal accessed inside the callback, re-running the callback whenever those signals change. Returns a cleanup function to stop watching.
 -->
コールバック内でアクセスされたシグナルの変更をサブスクライブし、それらのシグナルが変化するたびにコールバックを再実行します。監視 (watch) を停止するためのクリーンアップ関数を返します。
...
Unlike `data-wp-watch`, which is a directive tied to a DOM element's lifecycle, the `watch()` function is a programmatic API that can be used anywhere in your JavaScript code, independently of the DOM.
 -->
DOM 要素のライフサイクルに関連付けられたディレクティブ `data-wp-watch` とは異なり、`watch()` 関数はプログラムから利用する API であり、DOM に依存せず JavaScript コード内の任意の場所で使用できます。
...
// コールバック内でアクセスされたすべての state の変更を watch する (監視する)。
watch( () => {
	console.log( 'Counter is ' + state.counter );
} );

// コールバックは直ちに実行され、「Counter is 0」を出力する。
// `state.counter` が変更されるたびに再実行される。
```

<!-- 
The `watch()` function returns an `unwatch` function that, when called, stops the watcher and prevents further re-runs:
 -->
`watch()` 関数は `unwatch` 関数を返します。この関数を呼び出すと、監視が停止され、それ以降は再実行されなくなります。
...
The callback passed to `watch()` can also return a cleanup function. This cleanup function runs just before the callback re-executes due to a signal change, and also when the watcher is disposed of via `unwatch()`:
 -->
`watch()` に渡すコールバックは、クリーンアップ関数を返すこともできます。このクリーンアップ関数は、シグナルの変更によってコールバックが再実行される直前、および `unwatch()` によって監視が破棄される際に実行されます。
...
const unwatch = watch( () => {
	...
	// このクリーンアップ処理は次回の再実行の直前、または
	// `unwatch()` が呼び出されたときに実行されます。
	return () => {
